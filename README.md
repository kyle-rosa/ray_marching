...
### Differentiability
All scene construction, ray marching, and shader functions are implemented in PyTorch and are fully differentiable. This allows us to backproagate gradients through the rendering process, and optimise the scene parameters based on the pixel values of the rendered output image.

### Quaternions and 3D Rotations
Wherever possible, I've encoded rotations with unit quaternions. 

This is because the space of 3D rotations has a ``hole" in it, and paths through the space of 3D rotations can become knotted on this hole in a way that obstructs gradient descent algorithms. For example, consider a sequence $R_0, R_1, ..., R_N$ of rotations where $R_0=R_N=I$ and each $R_i^{-1}R_{i+1}$ is small. If the sequence traces out a path around the hole, it will be impossible to continuously deform it into a path that doesn't, such as the path where each $R_i$ is the identity. This phenomenon doesn't occur if the rotations are encoded with quaternions instead.
...
I've implemented a rudimentary constructive geometry system for signed distance functions, largely based on the work of Inigo Quilez [1]. This consists of *SDF primitives* with type signiture 

$$f:\mathbb{R}^3\to\mathbb{R},$$ 

some *SDF transformations* that map SDFs to SDFs and have type signiture 

$$F: (\mathbb{R}^3\to\mathbb{R}) \to (\mathbb{R}^3\to\mathbb{R}),$$

and *SDF operations* that map tuples of SDFs to SDFs, and have type signiture

$$\Phi: \bigcup_{n\in\mathbb{N}} (\mathbb{R}^3\to\mathbb{R})^n \to (\mathbb{R}^3\to\mathbb{R}). $$
...
### SDF Operations
Operations that compbine multiple input SDFs into a single new one:
  1. union, and
  2. smooth union.
...
The next step is to map this value to a colour, which we do by first applying the function 
...
2. Clean up:
    ...
    3. The pandas dependency is probably unnecessary and I should get rid of it.
3. Rendering:
